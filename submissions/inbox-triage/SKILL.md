...
| `pastEvents` | Subject starts with a **retrospective meeting-response** prefix (`Accepted:`, `Declined:`, `Tentative:`, `Meeting Forward Notification:` - or a localised prefix listed in `config.meetingResponsePrefixes`) AND the message is older than `pastEventMinAgeDays` (default 7 days) AND sender is a calendar system (Outlook / Exchange / Teams). These receipts record an action already taken, so they qualify regardless of the meeting's date. **Forward-looking** prefixes (`Canceled:`, `Cancelled:`, `Updated invitation:`) are deliberately left in the inbox: the meeting may be upcoming or a live recurring series, and Step 1 collects no calendar or body data to verify that. | `folders.pastEvents` (default `Inbox Triage/Past events`) |
...
5. **Re-verify protection state immediately before moving each message.** The plan was built from a Step 1 snapshot; time passes before the user approves, so a message can have become protected in between. For every approved message ID, re-fetch its current state (via the bound "get email" capability, or a fresh targeted "list emails" over the approved IDs) and **skip the move** if any protection now applies: it is flagged (`flag.flagStatus = flagged`), it is now unread and received within the unread-recent window (3 days), it gained a Confidential-or-above sensitivity label, its sender now falls in the active-thread window, or it is otherwise newly in scope of the Step 2 protection layer. Report skipped messages ("held back 3 that became active/flagged since the plan"). This keeps the Step 2 guarantee true at execution time, not just at planning time. If the re-fetch capability is unavailable, say so and ask the user to confirm before moving on stale state.
